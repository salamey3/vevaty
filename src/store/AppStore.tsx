...
// The moderation call used to build a payload here: six gallery photos and
// two randomly-sampled spin frames, each re-encoded to base64, so that the
// edge function could judge them. It judges the listing's own stored photos
// now (@MEDIA.md, "The check believed its caller"), which is both the point
// -- what was checked and what was published were two different things --
// and a second upload of every photo that a seller no longer pays for. The
// sampling that lived here moved into the function, where the frames it
// picks are the ones actually on the row.
    ...
    async (l: ListingInput) => {
      ...
      mediaLanded.then(
        async () => {
          ...
          // a post.
          //
          void triggerListingModeration(listingId);
  ...
  const updateListing = useCallback(
    async (id: string, l: ListingInput, opts: ListingSaveOptions = {}): Promise<ListingSaveResult> => {
      ...
      if (willModerate && blockedReason() === null) {
        void triggerListingModeration(id);
