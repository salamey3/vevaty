...
// Reactive moderation: listings publish immediately (no pre-publish approval
// gate -- see project notes), so this screen is the admin's after-the-fact
// tool to browse every listing regardless of status/owner (RLS's normal
// "active or mine" rule doesn't apply here -- "admins can view all listings"
// covers this screen specifically) and unpublish/relist/mark-sold anything.

type Status = 'draft' | 'active' | 'sold' | 'expired' | 'removed';
const STATUSES: Status[] = ['draft', 'active', 'sold', 'expired', 'removed'];
...
export default function AdminModerationScreen() {
  ...
  const [filter, setFilter] = useState<'all' | 'reported' | Status>('all');
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [{ data: listingRows, error: listErr }, { data: openReports, error: repErr }] = await Promise.all([
        supabase
          .from('listings')
          .select('id,title_en,price,status,district,created_at,seller_id,category_id,seller:profiles(full_name),photos:listing_photos(url,kind)')
  ...
  const setStatus = async (row: ModListing, next: Status) => {
    setBusyId(row.id);
    try {
      const { error: updErr } = await supabase.from('listings').update({ status: next }).eq('id', row.id);
              ...
              <View key={row.id} style={styles.card}>
                ...
                {expanded && (
                  <View style={styles.actions}>
                    ...
                    {STATUSES.filter((s) => s !== row.status && s !== 'removed').map((s) => (
                      <Pressy
                        key={s}
