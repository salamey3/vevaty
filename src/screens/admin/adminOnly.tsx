...
// the server refuses every admin write to anyone but an unlocked admin
// session -- but the admin area is meant to be invisible to members, not
// merely inert.
//
...
//
// While the server says the session is locked, the page is not rendered at
// all (AdminLockedBackdrop explains why), and comes back fresh once the code
// has been entered.
//
...
export function adminOnly<P extends object>(Page: React.ComponentType<P>): React.ComponentType<P> {
  function AdminOnly(props: P) {
    const { isAdmin, sessionLocked } = useSettings();
    if (!isAdmin) return <AdminGateScreen />;
    if (sessionLocked) return <AdminLockedBackdrop />;
    return <Page {...props} />;
  }
