...
export default function HomeScreen() {
  ...
  // search bar can expand into a "clean" search mode, independent of
  // `query` itself (this flips the instant the field is tapped, before any
  // text is typed -- `query` alone already governs when results replace
  ...
  const mobileGreetingSearchRow = (
    <View style={[styles.mobileGreetingRow, mirrorRow(isRTL)]}>
      {/* Hidden (not just visually, unmounted) while the search input has
          focus, rather than animated out -- removing it is what lets
          mobileSearchRow's own flex: 1 claim the freed width and expand
          leftward to fill the row, exactly the way it already claims
          whatever a short name leaves unused. */}
      {!searchFocused && (
